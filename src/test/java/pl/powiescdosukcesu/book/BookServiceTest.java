...
public class BookServiceTest {
	...
	public void setup() {
		...
		AppUser user = new AppUser("test","test","test","test","test@test.pl");
		book = Book.builder()
				.id(1)
				.title("Harry Potter")
	...
	@Test
    public void whenNoKeywordOrDateAtHandThenShouldReturnAll(){

	    //given
        given(bookRep.findAll((Pageable) null)).willReturn(new PageImpl<Book>(books));

        assertThat(bookService.getBooks(null,"","").getTotalElements()).isEqualTo(2);
    }
    ...
    public void whenSavingBookThenShouldReturnCorrectTitle(){
	    ...
	    given(appUserService.getUser("test"))
                .willReturn(new AppUser("test",
                                        "test",
                                        "test",
                                        "tester",
                                        "test@hg.pl"));
